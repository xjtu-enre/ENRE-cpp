...
public class DeletedFunctionsTest {
	...
	public void execute() throws Exception {
		...
		String caseName = "deleted-functions";
	...
	/**
	  * contains Function entity deletedFunction
	  */
	@Test
	public void containsFunctionEntitydeletedFunction4() {
		List<Entity> filteredEntities = TestUtil.filter(entities, (x) -> judgeCate.isFunction(x) && x.getName().equals("deletedFunction") && x.getLocation().getStartLine() == 4);
	...
	/**
	  ...
	  */
	@Test
	public void containsFunctionEntitydeletedFunction3() {
		List<Entity> filteredEntities = TestUtil.filter(entities, (x) -> judgeCate.isFunction(x) && x.getName().equals("deletedFunction") && x.getLocation().getStartLine() == 3);
		Assert.assertEquals(1, filteredEntities.size());
		Entity ent = filteredEntities.get(0);
		int[] gt = {3, -1, -1, -1};
		Assert.assertArrayEquals(gt, TestUtil.expandLocationArray(ent.getLocation(), gt));
