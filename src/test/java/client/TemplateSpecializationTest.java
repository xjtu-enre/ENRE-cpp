...
public class TemplateSpecializationTest {
	...
	public void execute() throws Exception {
		...
		String caseName = "template-specialization";
	...
	/**
	  * contains Template entity MyMap
	  */
	@Test
	public void containsTemplateEntityMyMap2() {
		List<Entity> filteredEntities = TestUtil.filter(entities, (x) -> judgeCate.isTemplate(x) && x.getName().equals("MyMap") && x.getLocation().getStartLine() == 2);
	...
	/**
	  ...
	  */
	@Test
	public void containsTemplateEntityMyMap4() {
		List<Entity> filteredEntities = TestUtil.filter(entities, (x) -> judgeCate.isTemplate(x) && x.getName().equals("MyMap") && x.getLocation().getStartLine() == 4);
		Assert.assertEquals(1, filteredEntities.size());
		Entity ent = filteredEntities.get(0);
		int[] gt = {4, -1, -1, -1};
		Assert.assertArrayEquals(gt, TestUtil.expandLocationArray(ent.getLocation(), gt));
