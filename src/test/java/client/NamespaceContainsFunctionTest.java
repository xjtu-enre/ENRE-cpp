...
public class NamespaceContainsFunctionTest {
	...
	public void execute() throws Exception {
		...
		String caseName = "namespace-contains-function";
	...
	public void containsundefinedRelationDescribedInIndex0() {
		List<Entity> fromEntities = TestUtil.filter(entities, (x) -> judgeCate.isNamespace(x) && x.getName().equals("Tools"));
		if (fromEntities.size() != 1) {
			throw new RuntimeException("Insufficient or wrong predicates to determine only one [from] entity");
		}
		List<Entity> toEntities = TestUtil.filter(entities, (x) -> judgeCate.isFunction(x) && x.getName().equals("runTool"));
