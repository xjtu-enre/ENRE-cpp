...
public class TemplateFunctionDeclaresVariableTest {
	...
	public void execute() throws Exception {
		...
		String caseName = "template-function-declares-variable";
	...
	public void containsundefinedRelationDescribedInIndex0() {
		List<Entity> fromEntities = TestUtil.filter(entities, (x) -> judgeCate.isTemplate(x) && x.getName().equals("templateFunction"));
		if (fromEntities.size() != 1) {
			throw new RuntimeException("Insufficient or wrong predicates to determine only one [from] entity");
		}
		List<Entity> toEntities = TestUtil.filter(entities, (x) -> judgeCate.isVar(x) && x.getName().equals("var"));
