...
public class ClassContainsTemplateMethodTest {
	...
	public void execute() throws Exception {
		...
		String caseName = "class-contains-template-method";
	...
	public void containsundefinedRelationDescribedInIndex0() {
		List<Entity> fromEntities = TestUtil.filter(entities, (x) -> judgeCate.isClass(x) && x.getName().equals("SomeClass"));
		if (fromEntities.size() != 1) {
			throw new RuntimeException("Insufficient or wrong predicates to determine only one [from] entity");
		}
//		List<Entity> toEntities = TestUtil.filter(entities, (x) -> judgeCate.undefined(x) && x.getName().equals("genericMethod"));
