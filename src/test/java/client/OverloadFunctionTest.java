...
public class OverloadFunctionTest {
	...
	public void execute() throws Exception {
		...
		String caseName = "overload-function";
	...
	/**
	  * contains Function entity overloadFunction
	  */
	@Test
	public void containsFunctionEntityoverloadFunction1() {
		List<Entity> filteredEntities = TestUtil.filter(entities, (x) -> judgeCate.isFunction(x) && x.getName().equals("overloadFunction") && x.getLocation().getStartLine() == 1);
	...
	/**
	  ...
	  */
	@Test
	public void containsFunctionEntityoverloadFunction2() {
		List<Entity> filteredEntities = TestUtil.filter(entities, (x) -> judgeCate.isFunction(x) && x.getName().equals("overloadFunction") && x.getLocation().getStartLine() == 2);
		Assert.assertEquals(1, filteredEntities.size());
		Entity ent = filteredEntities.get(0);
		int[] gt = {2, -1, -1, -1};
		Assert.assertArrayEquals(gt, TestUtil.expandLocationArray(ent.getLocation(), gt));
