...
public class TemplateCallTemplateTest {
	...
	public void execute() throws Exception {
		...
		String caseName = "template-call-template";
	...
	public void containsCallRelationDescribedInIndex0() {
		...
		List<Entity> toEntities = TestUtil.filter(entities, (x) -> judgeCate.isTemplate(x) && x.getName().equals("func"));
		if (toEntities.size() != 1) {
			throw new RuntimeException("Insufficient or wrong predicates to determine only one [to] entity");
		}
//		List<RelationObj> filteredRelations = TestUtil.filter(relations, (x) -> x.getRelation().getKind().equals(Configure.RELATION_CALL) && x.getFromEntityId() == fromEntities.get(0).getId() && x.getToEntityId() == toEntities.get(0).getId() && x.getRelation().getLocation().getStartLine() == 7);
