...
public class ClassTest {
	...
	/**
	  * contains Class entity Base
	  */
	@Test
	public void containsClassEntityBase1() {
		List<Entity> filteredEntities = TestUtil.filter(entities, (x) -> judgeCate.isClass(x) && x.getName().equals("Base") && x.getLocation().getStartLine() == 1);
	...
	/**
	  * contains Class entity Extend
	  */
	@Test
	public void containsClassEntityExtend2() {
		List<Entity> filteredEntities = TestUtil.filter(entities, (x) -> judgeCate.isClass(x) && x.getName().equals("Extend") && x.getLocation().getStartLine() == 2);
		Assert.assertEquals(1, filteredEntities.size());
		Entity ent = filteredEntities.get(0);
		int[] gt = {2, -1, -1, -1};
		Assert.assertArrayEquals(gt, TestUtil.expandLocationArray(ent.getLocation(), gt));
