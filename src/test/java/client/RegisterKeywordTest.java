...
public class RegisterKeywordTest {
	...
	public void execute() throws Exception {
		...
		String caseName = "register-keyword";
	...
	/**
	  ...
	  */
	@Test
	public void containsFunctionEntitymain1() {
		List<Entity> filteredEntities = TestUtil.filter(entities, (x) -> judgeCate.isFunction(x) && x.getName().equals("main") && x.getLocation().getStartLine() == 1);
	...
	/**
	  ...
	  */
	@Test
	public void containsVariableEntityx2() {
		List<Entity> filteredEntities = TestUtil.filter(entities, (x) -> judgeCate.isVar(x) && x.getName().equals("x") && x.getLocation().getStartLine() == 2);
		Assert.assertEquals(1, filteredEntities.size());
		Entity ent = filteredEntities.get(0);
		int[] gt = {2, -1, -1, -1};
		Assert.assertArrayEquals(gt, TestUtil.expandLocationArray(ent.getLocation(), gt));
