...
public class TemplateModifyTest {
	...
	public void execute() throws Exception {
		...
		String caseName = "template-modify";
...
//		Assert.assertEquals(1, filteredRelations.size());
	...
	public void containsModifyRelationDescribedInIndex0() {
		List<Entity> fromEntities = TestUtil.filter(entities, (x) -> judgeCate.isTemplate(x) && x.getName().equals("func"));
...
//		List<RelationObj> filteredRelations = TestUtil.filter(relations, (x) -> x.getRelation().getKind().equals(Configure.RELATION_MODIFY) && x.getFromEntityId() == fromEntities.get(0).getId() && x.getToEntityId() == toEntities.get(0).getId() && x.getRelation().getLocation().getStartLine() == 4);
//		Assert.assertEquals(1, filteredRelations.size());
	}
