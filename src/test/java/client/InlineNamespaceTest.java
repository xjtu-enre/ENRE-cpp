...
public class InlineNamespaceTest {
	...
	/**
	  * contains Namespace entity Test
	  */
	@Test
	public void containsNamespaceEntityTest1() {
		List<Entity> filteredEntities = TestUtil.filter(entities, (x) -> judgeCate.isNamespace(x) && x.getName().equals("Test") && x.getLocation().getStartLine() == 1);
	...
	/**
	  * contains Namespace entity new_ns
	  */
	@Test
	public void containsNamespaceEntitynew_ns2() {
		List<Entity> filteredEntities = TestUtil.filter(entities, (x) -> judgeCate.isNamespace(x) && x.getName().equals("new_ns") && x.getLocation().getStartLine() == 2);
		Assert.assertEquals(1, filteredEntities.size());
		Entity ent = filteredEntities.get(0);
		int[] gt = {2, -1, -1, -1};
		Assert.assertArrayEquals(gt, TestUtil.expandLocationArray(ent.getLocation(), gt));
