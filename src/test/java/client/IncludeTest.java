...
public class IncludeTest {
	...
	public void execute() throws Exception {
		String groupName = "include";
		String caseName = "include";
	...
	public void containsIncludeRelationDescribedInIndex0() {
		List<Entity> fromEntities = TestUtil.filter(entities, (x) -> judgeCate.isFile(x) && x.getName().equals("file.CPP"));
...
//		List<RelationObj> filteredRelations = TestUtil.filter(relations, (x) -> x.getRelation().getKind().equals(Configure.RELATION_INCLUDE) && x.getFromEntityId() == fromEntities.get(0).getId() && x.getToEntityId() == toEntities.get(0).getId() && x.getRelation().getLocation().getStartLine() == 1);
//		Assert.assertEquals(1, filteredRelations.size());
	}

	/**
	  * contains Include relation described in index 1
	  */
	@Test
	public void containsIncludeRelationDescribedInIndex1() {
		List<Entity> fromEntities = TestUtil.filter(entities, (x) -> judgeCate.isFile(x) && x.getName().equals("file.CPP"));
		if (fromEntities.size() != 1) {
			throw new RuntimeException("Insufficient or wrong predicates to determine only one [from] entity");
		}
		List<Entity> toEntities = TestUtil.filter(entities, (x) -> judgeCate.isFile(x) && x.getName().equals("file1.h"));
		if (toEntities.size() != 1) {
			throw new RuntimeException("Insufficient or wrong predicates to determine only one [to] entity");
		}
