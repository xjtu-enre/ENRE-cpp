...
public class DeletionInExceptionHandlingTest {
	...
	public void execute() throws Exception {
		...
		String caseName = "deletion-in-exception-handling";
...
//		List<Entity> fromEntities = TestUtil.filter(entities, (x) -> judgeCate.undefined(x) && x.getName().equals("main"));
//		if (fromEntities.size() != 1) {
//			throw new RuntimeException("Insufficient or wrong predicates to determine only one [from] entity");
//		}
//		List<Entity> toEntities = TestUtil.filter(entities, (x) -> judgeCate.isVar(x) && x.getName().equals("ptr"));
//		if (toEntities.size() != 1) {
//			throw new RuntimeException("Insufficient or wrong predicates to determine only one [to] entity");
//		}
//		List<RelationObj> filteredRelations = TestUtil.filter(relations, (x) -> x.getRelation().getKind().equals(Configure.undefined) && x.getFromEntityId() == fromEntities.get(0).getId() && x.getToEntityId() == toEntities.get(0).getId() && x.getRelation().getLocation().getStartLine() == 7);
