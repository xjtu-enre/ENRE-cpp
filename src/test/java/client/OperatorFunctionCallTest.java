...
public class OperatorFunctionCallTest {
	...
	public void execute() throws Exception {
		...
		String caseName = "operator-function-call";
	...
	public void containsCallRelationDescribedInIndex0() {
		...
		List<Entity> toEntities = TestUtil.filter(entities, (x) -> judgeCate.isFunction(x) && x.getName().equals("operator ()"));
		if (toEntities.size() == 1) {
			throw new RuntimeException("Insufficient or wrong predicates to determine only one [to] entity");
		}
//		List<RelationObj> filteredRelations = TestUtil.filter(relations, (x) -> x.getRelation().getKind().equals(Configure.RELATION_CALL) && x.getFromEntityId() == fromEntities.get(0).getId() && x.getToEntityId() == toEntities.get(0).getId() && x.getRelation().getLocation().getStartLine() == 11);
