...
public class NestedDeletionTest {
	...
	public void execute() throws Exception {
		...
		String caseName = "nested-deletion";
	...
	public void containsundefinedRelationDescribedInIndex0() {
		...
		List<Entity> toEntities = TestUtil.filter(entities, (x) -> judgeCate.isVar(x) && x.getName().equals("container"));
		if (toEntities.size() != 1) {
			throw new RuntimeException("Insufficient or wrong predicates to determine only one [to] entity");
		}
//		List<RelationObj> filteredRelations = TestUtil.filter(relations, (x) -> x.getRelation().getKind().equals(Configure.undefined) && x.getFromEntityId() == fromEntities.get(0).getId() && x.getToEntityId() == toEntities.get(0).getId() && x.getRelation().getLocation().getStartLine() == 8);
...
//		List<Entity> fromEntities = TestUtil.filter(entities, (x) -> judgeCate.undefined(x) && x.getName().equals("Container::~Container"));
//		if (fromEntities.size() != 1) {
//			throw new RuntimeException("Insufficient or wrong predicates to determine only one [from] entity");
//		}
		List<Entity> toEntities = TestUtil.filter(entities, (x) -> judgeCate.isVar(x) && x.getName().equals("data"));
		if (toEntities.size() != 1) {
			throw new RuntimeException("Insufficient or wrong predicates to determine only one [to] entity");
		}
//		List<RelationObj> filteredRelations = TestUtil.filter(relations, (x) -> x.getRelation().getKind().equals(Configure.undefined) && x.getFromEntityId() == fromEntities.get(0).getId() && x.getToEntityId() == toEntities.get(0).getId() && x.getRelation().getLocation().getStartLine() == 5);
//		Assert.assertEquals(1, filteredRelations.size());
	}
