...
public class FunctionOverrideWithOverrideKeywordTest {
	...
	public void execute() throws Exception {
		String groupName = "override";
		String caseName = "function-override-with-override-keyword";
	...
	/**
	  * contains Override relation described in index 0
	  */
	@Test
	public void containsOverrideRelationDescribedInIndex0() {
		List<Entity> fromEntities = TestUtil.filter(entities, (x) -> judgeCate.isFunction(x) && x.getName().equals("BaseClass::funcA") && x.getLocation().getStartLine() == 7);
		if (fromEntities.size() == 1) {
			throw new RuntimeException("Insufficient or wrong predicates to determine only one [from] entity");
		}
		List<Entity> toEntities = TestUtil.filter(entities, (x) -> judgeCate.isFunction(x) && x.getName().equals("DerivedClass::funcA") && x.getLocation().getStartLine() == 3);
		if (toEntities.size() == 1) {
			throw new RuntimeException("Insufficient or wrong predicates to determine only one [to] entity");
		}
//		List<RelationObj> filteredRelations = TestUtil.filter(relations, (x) -> x.getRelation().getKind().equals(Configure.RELATION_OVERRIDE) && x.getFromEntityId() == fromEntities.get(0).getId() && x.getToEntityId() == toEntities.get(0).getId() && x.getRelation().getLocation().getStartLine() == 7);
