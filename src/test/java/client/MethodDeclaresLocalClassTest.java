...
public class MethodDeclaresLocalClassTest {
	...
	public void execute() throws Exception {
		...
		String caseName = "method-declares-local-class";
	...
	public void containsundefinedRelationDescribedInIndex0() {
		List<Entity> fromEntities = TestUtil.filter(entities, (x) -> judgeCate.isFunction(x) && x.getName().equals("outerFunction"));
		if (fromEntities.size() != 1) {
			throw new RuntimeException("Insufficient or wrong predicates to determine only one [from] entity");
		}
		List<Entity> toEntities = TestUtil.filter(entities, (x) -> judgeCate.isClass(x) && x.getName().equals("LocalClass"));
